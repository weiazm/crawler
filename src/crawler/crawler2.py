...
def getF0Content(soup):
    # f0的内容
    soup = soup.find(id="topic_detail_main").find(class_="conmain").find(id="maxwrap-maintopic").find(
        class_="clearfix contstxt outer-section")
    # 返回楼主uid，楼层,发帖时间，帖子内容,回复楼层
    try:
        return soup["uid"], 0, soup["data-time"], soup.find(class_="conright fr").find(class_="rconten").find(
            class_="conttxt").find(class_="w740").get_text().strip(), 0
    except:
        return soup["uid"], 0, soup["data-time"], soup.find(class_="conright fr").find(class_="rconten").find(
            class_="conttxt tpt").get_text().strip(), 1
...
def getAllFContent(list):
    ...
    for soup in list:
        ...
        floorNum = soup["rf"]
        content = soup.find(class_="conright fl").find(class_="rconten").find(class_="x-reply font14").find(
            class_="w740")
        content2 = None
        # face = None
        if content == None:
...
for x in range(24556, 30000):
    try:
        ...
        try:
            ...
            logging.error(
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' id=' + str(x) + ' HTTPError = ' + str(
        ...
        insertIntoDatabase(conn, unicode(x), unicode(links[0]), unicode(title), unicode(F0[0]), unicode(F0[1]),
                           unicode(F0[2]), unicode(F0[3]), unicode(F0[4]))
        for link in links[1]:
        ...
        for soup in soups:
            for lis in getAllFContent(getAllFContentList(soup)):
                ...
                insertIntoDatabase(conn, unicode(x), unicode(links[0]), unicode(title), unicode(lis[0]),
                                   unicode(lis[1]), unicode(lis[2]), unicode(lis[3]), unicode(lis[4]))
    except Exception, e:
        logging.error(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' id=' + str(x) + ' Error = ' + str(
            e) + u"未知错误发生:" + url)
        conn.rollback()
        continue
    else:
