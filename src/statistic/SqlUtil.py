...
class MysqlOperator(object):
    @classmethod
    def updateClickNumAndReplyNum(cls, conn, bbs_id, clickNum, replyNum):
        cursor = conn.cursor()
        cursor.execute('update forum_links_user set click_num =%s,reply_num =%s where bbs_id =%s',
    ...
    def selectLinkById(self, id):
        cursor = self.__conn.cursor()
        cursor.execute('select * from forum_links_user where id = %s', [id])
    ...
    def insertUser(self, user):
        ...
        cursor = self.__conn.cursor()
        cursor.execute(
            "INSERT INTO `refactor_crawler`.`user`(`uid`,`sex`,`nickname`,`location`,`auth`,`auth_cars`,`points`,`level`,`follows`,`num_of_follows`,`fans`,`num_of_fans`,`create_time`,`num_of_main_bbs`,`num_of_elite_bbs`,`num_of_reply`,`num_of_reply_self`,`num_of_reply_others`,`first_post_time`,`last_post_time`,`avg_num_of_bbs`,`if_oil_consule`,`if_comment`,`if_cost`,`post_bbs_category_text`,`reply_bbs_category_text`)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            [u.uid, u.sex, u.nickname, u.location, u.auth, u.auth_cars, u.points, u.level, u.follows, u.num_of_follows,
             u.fans,
